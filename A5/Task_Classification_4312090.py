...
def random_forest_experiment(n_estimators, min_samples_leaf, X, y):
    ...
    classifier = RandomForestClassifier(n_estimators=n_estimators, min_samples_leaf=min_samples_leaf, random_state=2024)
    ...
    cv_10 = KFold(n_splits = 10, shuffle=True, random_state = 2024)
    scores = cross_val_score(classifier, X, y, cv=cv_10, scoring='accuracy')
...
def get_best_hyperparameters(n_estimators_list, min_samples_leaf_list, X, y):
    ...
    for n_estimators in n_estimators_list:
        for min_samples_leaf in min_samples_leaf_list:
            result_dic = dict.fromkeys(['n_estimators', 'min_samples_leaf', 'accuracy'])
...
def manually_entered_best_params_and_accuracy():
    ...
    best_accuracy = 0.7096  # Example accuracy, to be replaced with your achieved accuracy
